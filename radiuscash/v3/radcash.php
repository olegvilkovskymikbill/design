...
$mac_onu=$mysqlconnect->query("SELECT u.uid, u.local_ip, local_mac, u.gid, u.sectorid, u.speed_rate, u.speed_burst, r.callingstationid FROM radacct r, users u, packets t WHERE r.radacctid = ( SELECT MAX( r.radacctid ) FROM radacct r WHERE r.uid = u.uid ) AND r.callingstationid LIKE \"%:%\" AND u.deposit + u.credit >=0 - t.razresh_minus AND u.blocked =0 AND u.gid = t.gid");
$mac_type="local_mac";
} else {
$users=$mysqlconnect->query("SELECT u.uid, u.local_ip, u.local_mac, u.gid, u.sectorid, u.speed_rate, u.speed_burst FROM users u, packets t WHERE local_mac IS NOT NULL AND u.deposit + u.credit >=0 - t.razresh_minus AND u.blocked =0 AND u.gid = t.gid");
$mac_type="callingstationid";
}
