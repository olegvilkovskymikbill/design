...
gdrive --access-token $Gdrive_access_token upload $PACH_FOR_BACKUP_TO_DISK/$FILENAME >>$LOG
fi
}

Func_dropbox_uploader(){
if [ "$BACKUP_TO_DROPBOX_UPLOADER" -ne 0 ]; then
  $HOME_DIR/dropbox_uploader.sh upload $PACH_FOR_BACKUP_TO_DISK/$FILENAME $DIR_BACKUP_FOR_WEBDISK/$FILENAME_WEBDISK 2>>$LOG
fi
}
